# MEMORY-SYSTEM.md — Corrections & Rules Log

Tracks repeated corrections. When a correction appears 3+ times, it becomes a candidate for permanent rule.

## Corrections Log
| Date | Issue | Correction | Count |
|------|-------|------------|-------|
| 2026-03-21 | File created | Initial structure | 1 |

## Maintenance Log
| Date | Action | Notes |
|------|--------|-------|
| 2026-03-22 | Weekly maintenance | No duplicates, no stale items, archive dir created |

## Permanent Rules
*(Auto-promoted from corrections with 3+ entries after David approval)*

1. **NO `pkill`, `killall`, or broad process termination** — causes gateway crash
2. **NO signal-sending commands** (`kill -9`, `kill -TERM`) — causes gateway disconnect

## Pending Rule Candidates
*(None yet — will populate as corrections accumulate)*
