...
async function main() {
  ...
  // Continue from where we left off - around contact 80
  // Batch 2 sent ~29 emails before interruption
  // Start from contact 80 to be safe
  const startIdx = 80;
  const batchSize = 50;
  const batch = contacts.slice(startIdx, startIdx + batchSize);
  
  console.log('Total contacts:', contacts.length);
  console.log('Continuing from contact', startIdx + 1, '...\n');
  ...
  console.log('Remaining:', contacts.length - startIdx - sent);
  console.log('========================================');
}
